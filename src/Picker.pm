...
sub printAlignments {
    ...
    for(my $i=0; $i<$numRows; ++$i) {
        ...
        if ($numSames>0) {
            if ($numSames>$maxSubRows) {
                print $fh "=(",$numSames,")\n";
            }
            printAlignments($fh, $alignRef->{'samereadspan'}, '=', $maxSubRows);
        } elsif ($numSubsets>0) {
            if ($numSubsets>$maxSubRows) {
                print $fh "~(",$numSubsets,")\n";
            }
            printAlignments($fh, $alignRef->{'subset'}, '~', $maxSubRows);
            
            if ($numSimilars>0) {
                if ($numSimilars>$maxSubRows) {
                    print $fh "-(",$numSimilars,")\n";
                }
                printAlignments($fh, $alignRef->{'similar'}, '-', $maxSubRows);
            }
        } elsif ($numSimilars>0) {
            if ($numSimilars>$maxSubRows) {
                print $fh "-(",$numSimilars,")\n";
...
sub processAlignments {
    ...
    printf $fhLog "%s Processing read %s (%d,%d)", utilities::getTimeStamp(), $currReadRef->{read}, $unfilteredCounts, $numOfAligns;
    ...
    if (0==$numOfAligns) {
        ...
        print $fhAlign "# ", $currReadRef->{readsize}, "\t", $currReadRef->{read}, "\t", "{!!!}";
        printf $fhAlign "\talign(%d,%d)\tseed(%d)\tnonseed(%d)", $reportMetrics{'#uf_aligns'}, $reportMetrics{'#aligns'}, $reportMetrics{'#seeds'}, $reportMetrics{'#nonseeds'} if (0!=$G_REPORT_METRICS);
        print $fhAlign "\n";
        print $fhAlign "# ", join("\t", @G_ReportCols), "\n";
        print $fhAlign "\n";
        ...
        print $fhAlign "# ", $currReadRef->{readsize}, "\t", $currReadRef->{read}, "\t", "(1)";
        printf $fhAlign "\talign(%d,%d)\tseed(%d)\tnonseed(%d)", $reportMetrics{'#uf_aligns'}, $reportMetrics{'#aligns'}, $reportMetrics{'#seeds'}, $reportMetrics{'#nonseeds'} if (0!=$G_REPORT_METRICS);
        print $fhAlign "\n";
        print $fhAlign "# ", join("\t", @G_ReportCols), "\n";
        printAlignments ($fhAlign, $alignmentsRef, undef, -1, $G_MAX_REPORT_SUBROW);
        print $fhAlign "\n";
        ...
        if (scalar(@{$localAlignsRef})>0) {
            ...
            printf $fhLog " seed(%d) nonseed(%d)", $numSeeds, $numNonSeeds;
            ...
            if ($maxCoverageFraction>=$G_MIN_FINAL_READ_COVERAGE) {
                ...
                foreach my $seedRef (@collapsedSeeds) {
                    ...
                    if (0!=$candidatesReduction) {
                        printf $fhLog " reduced(R,%d,%d)..", $numRightCandidates, $G_MAX_EXTEND_CANDIDATES if ($G_MAX_EXTEND_CANDIDATES<$numRightCandidates);
                        printf $fhLog " reduced(L,%d,%d)..", $numLeftCandidates, $G_MAX_EXTEND_CANDIDATES if ($G_MAX_EXTEND_CANDIDATES<$numLeftCandidates);
                    }
                    ...
                    if ($currCount>0) {
                        if (0!=isMultiLoci(\@candidates)) {
                            print $fhLog " multi-loci detected..";
                            print $fhLog " exceeded $G_MAX_EXTEND_SEED_TIME extension time.." if (0!=$extendTimeExceeded);
                            last;
                        } elsif ($currCount>$G_MAX_REPORT_CANDIDATES) {
                            print $fhLog " exceeded $G_MAX_REPORT_CANDIDATES candidates..";
                            print $fhLog " exceeded $G_MAX_EXTEND_SEED_TIME extension time.." if (0!=$extendTimeExceeded);
                            last;
                        }
                        ...
                        print $fhLog " exceeded $G_MAX_EXTEND_SEED_TIME extension time..";
                        last;
                    }
                }
                ...
                printf $fhLog " mpc(%.2f)", $maxCoverageFraction*100.0;
            }
        } else {
            $reportMetrics{'#seeds'} = 0;
            $reportMetrics{'#nonseeds'} = 0;
        }
        print $fhLog " candidates(", scalar(@candidates), ")";
        if (0!=$G_REPORT_IOTIME) {
            printf $fhLog " %sio(%d)", ($ioTime>$G_REPORT_MAX_IOTIME_SEC?"*":""), $ioTime;
        }
        if (0!=$G_REPORT_PROCESSTIME) {
            my $endTime = time();
            my $processTime = $endTime - $startTime;
            printf $fhLog " %sprocess(%d)", ($processTime>$G_REPORT_MAX_PROCESSTIME_SEC?"*":""), $processTime;
        }

		# let's report the candidate(s)
        my $numCandidates = scalar(@candidates);
        print $fhAlign "# ", $currReadRef->{readsize}, "\t", $currReadRef->{read}, "\t", ($numCandidates<=1 ? "(X)":"[ ]");
        printf $fhAlign "\talign(%d,%d)\tseed(%d)\tnonseed(%d)", $reportMetrics{'#uf_aligns'}, $reportMetrics{'#aligns'}, $reportMetrics{'#seeds'}, $reportMetrics{'#nonseeds'} if (0!=$G_REPORT_METRICS);
        printf $fhAlign "\textExcess", if (0!=$extendTimeExceeded);
        print $fhAlign "\n";
        print $fhAlign "# ", join("\t", @G_ReportCols), "\n";
        ...
        if ($numCandidates>1) {
            ...
            foreach my $candidateRef (@candidates) {
                ...
                print $fhAlign "### candidate#",$candidateId,"/",$numCandidates,"\n";
                printAlignments ($fhAlign, $candidateRef, undef, -1, $G_MAX_REPORT_SUBROW);
            }
        } elsif (1==$numCandidates) {
            @{$candidates[0]} = sort { $a->{read}->{readstart}<=>$b->{read}->{readstart} } @{$candidates[0]};
            print $fhAlign "### candidate#1/1\n";
            printAlignments ($fhAlign, $candidates[0], undef, -1, $G_MAX_REPORT_SUBROW);
        } else {
            print $fhAlign "### NO candidate left\n";
        }
