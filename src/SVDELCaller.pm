...
my $G_DEL_MIN_SDIFF = 20; # sDiff >20, or >=21
my $G_DEL_MAX_QDIFF = 20; # qDiff <20, or <=19
...
sub new {
	...
	$parametersRef->{'_del_min_sdiff'} = $G_DEL_MIN_SDIFF if (!exists $parametersRef->{'_del_min_sdiff'});
	$parametersRef->{'_del_max_qdiff'} = $G_DEL_MAX_QDIFF if (!exists $parametersRef->{'_del_max_qdiff'});
	...
	# TODO: check if we have to remove homopolymer
	# TODO: keep the genomeSequences
